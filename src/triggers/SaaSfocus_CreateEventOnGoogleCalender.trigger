...
trigger SaaSfocus_CreateEventOnGoogleCalender on Event (after insert,after update) {
    if(Saasfocus_Global.GoogleSyncProces == True) return;
    ...
    for(Event eventObj : Trigger.new) {
        ...
        if(whatId.contains(keyPrefix)) ids.add(eventObj.id);
    }

    if(Trigger.isInsert) {
        if(ids != null) Saasfocus_GoogleCalenderHelper.processEvent(ids);
    }
